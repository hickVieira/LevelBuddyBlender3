...
# PROPERTIES
bpy.types.Scene.texel_density = bpy.props.IntProperty(
    name="Texel Density", default=128, step=128, min=8, max=512)
...
bpy.types.Scene.nudge_amount = bpy.props.FloatProperty(
    name="Nudge Amount", default=0.125)
...
class TextureBuddyPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if context.mode == 'EDIT_MESH' or context.mode == 'OBJECT':
            col = layout.column(align=True)
            col.label(icon="TEXTURE", text="Mapping")
            ...
            if context.mode == 'EDIT_MESH':
                row = layout.row(align=True)
                row.operator("object.texture_buddy_pin",
                             text="Pin UVs").p = False
                row.operator("object.texture_buddy_pin",
                             text="Un-Pin UVs").p = True
                col = layout.column(align=True)
                ...
                row = layout.row(align=True)
                row.operator("object.texture_buddy_nudge",
                             text="Left").dir = "LEFT"
                row.operator("object.texture_buddy_nudge",
                ...
                row = layout.row(align=True)
                row.operator("object.texture_buddy_nudge",
                             text="Horizontal").dir = "HORIZONTAL"
                row.operator("object.texture_buddy_nudge",
                             text="Vertical").dir = "VERTICAL"
                ...
                if f.select:
                    ...
                    if len(obj.data.materials) > matIndex:
                        if obj.data.materials[matIndex] is not None:
                            ...
                            # try:
                            #     textureWidth = context.active_object.data.materials[
                            #         matIndex].active_texture.image.size[0]
                            #     textureHeight = context.active_object.data.materials[
                            #         matIndex].active_texture.image.size[1]
                            # except:
                            #     pass
                            nX = f.normal.x
                            nY = f.normal.y
